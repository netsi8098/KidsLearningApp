...
function buildRig(scene: THREE.Object3D | null): Rig {
  ...
  if (import.meta.env.DEV) {
    ...
    /* Also parked on `window`, because a console line cannot be asserted
       against. The bind is entirely geometry-dependent — it classifies vertex
       ISLANDS by size and height — so it has to be re-checked whenever the
       world is rebuilt, and this pass rebuilt it three times between the code
       being written and merged. A number you can read back is the difference
       between "it still binds" and "it probably still binds". */
    (window as unknown as Record<string, unknown>).__worldLife = {
      bounds: bounds.length,
      verts: touched,
      clouds: !!cloud,
      byKind: bounds.reduce<Record<string, number>>((m, b) => {
